...
@Component
@Slf4j
public class StorageMessageReceiver {

    @Value("${spring.rabbitmq.listener.simple.acknowledge-mode:}")
    private String ackMode;

    @RabbitListener(queues = "${gcloud.service.storageNode}")
    @RabbitHandler
    public ReplyMessage process(GMessage message, Message msg, Channel channel) {
        ...
        try {
            ...
            log.error(String.format("message handle error, ex=%s, message=%s, msgId=%s, serviceId=%s, messageClass=%s", e.toString(), e.getMessage(), message.getMsgId(),
                    message.getServiceId(), message.getClass()), e);
    ...
    @RabbitListener(queues = "${gcloud.service.storageNode}" + "_async")
    @RabbitHandler
    public void proccessAsync(GMessage message, Message msg, Channel channel) {
        try {
            ...
            log.error(String.format("message handle error, ex=%s, message=%s, msgId=%s, serviceId=%s, messageClass=%s", ex.toString(), ex.getMessage(), message.getMsgId(),
                    message.getServiceId(), message.getClass()), ex);
        }
    }
