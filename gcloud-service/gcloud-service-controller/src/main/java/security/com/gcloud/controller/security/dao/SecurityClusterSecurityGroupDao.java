...
@Repository
public class SecurityClusterSecurityGroupDao extends JdbcBaseDaoImpl<SecurityClusterSecurityGroup, Integer> {

    public int deleteByClusterId(String clusterId){
        String sql = "delete from gc_security_cluster_securitygroup where cluster_id = ?";
        Object[] values = {clusterId};
        return this.jdbcTemplate.update(sql, values);
    }

    public int deleteHaByClusterId(String clusterId){
        String sql = "delete from gc_security_cluster_securitygroup where cluster_id = ? and network_type = ?";
