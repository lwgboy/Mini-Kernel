...
do_setip()
{
  ...
  if [ "$targetDns" != "" ];then
    ...
    echo $dnsList
    echo "dns-nameservers $dnsList" >> $ethFile
  fi
  ...
  echo "ifdown $targetEth --force"
  ifdown $targetEth --force
  
  if [ $timeout -gt 0  ];then
     echo "timeout -s 9 ${timeout}s ifup $targetEth --force"
     timeout -s 9 ${timeout}s ifup $targetEth --force
  else
     echo "ifup $targetEth --force"
     ifup $targetEth --force
  fi
  if [ $? -gt 0 ];then
    echo "do_del_eth_config $targetMac $targetEth"
    do_del_eth_config $targetMac $targetEth
...
do_setup_rc_local()
{
  ...
  if [ $? -gt 0 ];then
    ...
    if [ $? -gt 0 ];then
      ...
      sed -i s/^\ *python\ *\\/usr\\/lib\\/route-setup_debian.py/python\ \\/usr\\/share\\/gcloud\\/netTools\\/route_setup.py/ /etc/rc.local
...
#call after delete or add 
do_reset_dns()
{
  echo "reset dns"
  echo "dpkg -l | grep resolvconf"
  dpkg -l | grep resolvconf
  if [ $? -gt 0 ];then
     
    echo "resolvconf not exit, modify dns"
    ...
    if [ ! -f "$resolvConfPath" ];then
     echo "resolvConfPath file not exist"
     return 0;
    fi
    ...
    for newDns in `cat $netcfgPath | grep ^\ *dns-nameservers`
    do
      if [ "$newDns" != "dns-nameservers" ];then
         echo $newDnsList | grep $newDns -w > /dev/null
         if [ $? -gt 0 ];then
           newDnsList="${newDnsList} ${newDns}"
         fi
      fi
    done
    
    for oldDns in `cat $resolvConfPath | grep ^\ *nameserver`
    do
      if [ "$oldDns" != "nameserver" ];then
        echo $oldDnsList | grep $oldDns -w > /dev/null
        if [ $? -gt 0 ];then
          oldDnsList="${oldDnsList} ${oldDns}"
        fi
      fi
    done
    
    echo "newDnsList: "$newDnsList
    echo "oldDnsList: "$oldDnsList
    
    #add dns begin 
    addNewDnsList= 
    for nnDns in $newDnsList
    do
      echo $oldDnsList | grep $nnDns -w > /dev/null
      if [ $? -gt 0 ];then
        echo "new dns not exist, dns=$nnDns"
        addNewDnsList="${addNewDnsList} ${nnDns}"
      fi
    done
    echo "add dns" $addNewDnsList
    if [ "$addNewDnsList" != "" ];then
      for setDns in $addNewDnsList
      do
        echo "nameserver $setDns" >> $resolvConfPath
      done
    fi
    ...
    for ddDns in $oldDnsList
    do
      echo $newDnsList | grep $ddDns -w > /dev/null
      if [ $? -gt 0 ];then
        echo "old dns not exist, dns=$ddDns"
        delOldDnsList="${delOldDnsList} ${ddDns}"
      fi
    done
    echo "del dns" $delOldDnsList
    if [ "$delOldDnsList" != "" ];then
      for delDns in $delOldDnsList
      do
        sed -i /$delDns/d $resolvConfPath
      done
    fi
