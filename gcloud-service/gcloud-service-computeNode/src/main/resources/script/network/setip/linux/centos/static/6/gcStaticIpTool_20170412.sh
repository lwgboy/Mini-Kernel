...
do_check_udev_rule()
{
  rulePath=/lib/udev/rules.d/60-net.rules
  if [ -f "$rulePath" ];then
    echo "rule file exist"
    cat $rulePath | grep ^\ *ACTION==\"add\".*SUBSYSTEM==\"net\".*PROGRAM=\"/lib/udev/rename_device\".*NAME=\"\$result\"
    if [ $? -gt 0 ];then
      echo "rule not exit or name not set"
      cat $rulePath | grep ^\ *ACTION==\"add\".*SUBSYSTEM==\"net\".*PROGRAM=\"/lib/udev/rename_device\".*
      if [ $? -gt 0 ];then
        echo "rule not eixt"
        echo ACTION==\"add\", SUBSYSTEM==\"net\", PROGRAM=\"/lib/udev/rename_device\", RESULT==\"?*\", ENV{INTERFACE_NAME}=\"\$result\", NAME=\"\$result\" >> $rulePath
      else
        echo "rule eixt, but name not set"
        sed -i 's/ACTION=="add".*SUBSYSTEM=="net".*PROGRAM="\/lib\/udev\/rename_device".*/&,\ NAME="$result"/' $rulePath
      fi
