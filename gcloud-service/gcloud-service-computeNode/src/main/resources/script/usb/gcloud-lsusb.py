...
def print_usb(usbdev):
	for dev in usbdev.children:
		for  ifs  in  dev.interfaces:
			if ifs.protoname[0:12].find("Storage")>0 or ifs.protoname[0:15].find("Vendor Specific")>=0:
				print  "pid:%04x,vid:%04x,sn:%s,product:%s driver:%s,dev:%s"  %(dev.pid,dev.vid,dev.name[dev.name.rfind(" "):],dev.name[:dev.name.rfind(" ")],ifs.driver,ifs.devname)
		print_usb(dev)
