...
@Slf4j
@Handler
public class DestroyInstanceHandler extends AsyncMessageHandler<DestroyInstanceMsg> {
    ...
    @Override
    public void handle(DestroyInstanceMsg msg) {
        ...
        try{
            vmBaseNodeService.destroy(msg.getInstanceId());
            replyMsg.setSuccess(true);
        }catch (Exception ex){
            log.error("云服务器destroy异常", ex);
            replyMsg.setErrorCode(ErrorCodeUtil.getErrorCode(ex, "::云服务器destroy异常"));
